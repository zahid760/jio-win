            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto"> 
                            <form action="{{route('matka.game.bid', $game_id)}}" method="get">
                                <div class="d-flex">
                                    <input type="date" name="date-filter" class="form-control" id="date-filter" value="{{date('Y-m-d', strtotime($date))}}">
                ...
                <div class="card-body pt-0">
                    <ul class="nav nav-tabs" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" data-bs-toggle="tab" href="#close" role="tab" aria-selected="false" tabindex="-1">Close</a>
                        </li>
                    </ul>
                    <div class="tab-content">
                        ...
                        <div class="tab-pane py-1" id="close" role="tabpanel">
                            <div class="table-responsive">
                                <table class="table  mb-0 table-centered table-bordered">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            @foreach($game_mode as $row)
                                                <td class="p-1 align-top">
                                                    <table class="table mb-0 table-centered table-bordered">
                                                        <tr>
                                                            <td class="p-1 text-danger">Users :
                                                                @foreach($totalusers_close as $keyid => $user)
                                                                    @if($row->id === $keyid)
                                                                        {{$user ?? 0}}
                                                                    @endif
                                                                @endforeach
                                                            ...
                                                            <td class="p-1 text-success">Profit :
                                                                @foreach($totalpoints_close as $keyid => $point)
                                                                    @if($row->id === $keyid)
                                                                        {{$point ?? 0}}
                                                                    @endif
                                                                @endforeach
                                                        ...
                                                        @foreach($bid_no_close as $keyid => $bid)
                                                            @if($row->id === $keyid)
                                                                @foreach($bid as $keyno => $bidpoint)
                                                                <tr>
                                                                    <td class="p-1">{{$bidpoint['game_number']}}</td>
