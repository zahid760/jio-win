            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title">Game Rate Update</h4>
                        </div>
                    </div>
                </div>

                <div class="card-body pt-0">
                    <form action="{{ route('game_rate.update', $data->id) }}" method="post" id="game-form">
                        @csrf
                        @method('PUT')
                        <div class="row g-2">
                            <div class="col-md-6">
                                ...
                                <select name="category" class="form-select" id="gameCategory" required="">
                                    ...
                                    <option value="matka" {{$data->category == 'matka' ? 'selected' : ''}}>Matka</option>
                                    <option value="satta" {{$data->category == 'satta' ? 'selected' : ''}}>Satta</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="gamemode" class="form-select" id="gameMode" required="">
                                    ...
                                    <option value="{{$row->id}}" {{$data->gamemode == $row->id ? 'selected' : ''}}>{{$row->name}} - ({{$row->category}})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="bidding_rate" class="form-control" id="bidingRate" placeholder="Enter Biding Rate" value="{{$data->bidding_rate}}" required="">
                            </div>
                            <div class="col-md-6">
                                <label for="winningRate" class="form-label">Winning Rate <span class="text-danger">*</span></label>
                                <input type="text" name="winning_rate" class="form-control" id="winningRate" placeholder="Enter Winning Rate" value="{{$data->winning_rate}}" required="">
                            </div>
                            <div class="col-md-12">
