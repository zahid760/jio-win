            ...
            <div class="row">
                ...
                <div class="col-auto d-flex">
                    ...
                    <button class="btn d-flex align-items-center px-0 ms-1" type="button">
                        <a href="{{ route('notification') }}" class="text-dark">
        ...
        <div class="container-fluid">
            @foreach($matka_games as $game)
                @php
                    ...
                    $close_time = \Carbon\Carbon::parse($game->close_time)->subMinutes(5);
                    $close_time_spl_game = \Carbon\Carbon::parse($game->close_time)->addHour();
                ...
                @if(!in_array($current_day, $close_days))
                    @if($game->spl == 1)
                        <div class="row">
                            <div class="col-12">
                                @if($current_time > $close_time_spl && $current_time < $close_time_spl_game)
                                        ...
                                        <div class="card-body p-2">
                                            <div class="row">
                                                <div class="col-8">
                                                    ...
                                                    @if($current_time > $close_time_spl && $current_time < $close_time_spl_game)
                                                        <p class="text-danger mb-0 "><strong>{{ $game->result_spl->open ?? '***' }}-{{ strlen($game->result_spl->jodi ?? '') === 1 ? $game->result_spl->jodi . '*' : ($game->result_spl->jodi ?? '**') }}-{{ $game->result_spl->close ?? '***' }}</strong></p>
                                                    @else
                                                        <p class="text-danger mb-0 "><strong>{{ $game->result->open ?? '***' }}-{{ strlen($game->result->jodi ?? '') === 1 ? $game->result->jodi . '*' : ($game->result->jodi ?? '**') }}-{{ $game->result->close ?? '***' }}</strong></p>
                                                    @endif
                                                    <div class="row">
                                                        <div class="col-6">
                                                        ...
                                                        <div class="col-6">
                                                            <p class="mb-0 fs-12">Close Time:</p>
                                                            <p class="mb-0 fs-10">{{ formatTime($game->close_time) }}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-4">
                                                    ...
                                                    @elseif($current_time > $open_time)
                                                        <p class="text-success mb-0 fs-11 text-center">Close is Running</p>
    ...
    <section class="bottom-nav fixed-bottom bg-white py-1">
        <div class="container-fluid">
            <div class="row gx-2 flex-nowrap">
                ...
                <div class="col text-center">
                    <a href="{{route('get-support.index')}}" class="btn p-0">
